...
class LOBFeatureExtractor:
    ...
    def extract_lob_state_features(self, lob_snapshot: Dict) -> np.ndarray:
        ...
        """
        bid_prices = np.array(lob_snapshot.get("bid_prices", [0] * self.num_levels))[
            : self.num_levels
        ]
        bid_volumes = np.array(lob_snapshot.get("bid_volumes", [0] * self.num_levels))[
            : self.num_levels
        ]
        ask_prices = np.array(lob_snapshot.get("ask_prices", [0] * self.num_levels))[
            : self.num_levels
        ]
        ask_volumes = np.array(lob_snapshot.get("ask_volumes", [0] * self.num_levels))[
            : self.num_levels
        ]
        ...
        if len(bid_prices) < self.num_levels:
            bid_prices = np.pad(
                bid_prices, (0, self.num_levels - len(bid_prices)), "constant"
            )
            bid_volumes = np.pad(
                bid_volumes, (0, self.num_levels - len(bid_volumes)), "constant"
            )
        if len(ask_prices) < self.num_levels:
            ask_prices = np.pad(
                ask_prices, (0, self.num_levels - len(ask_prices)), "constant"
            )
            ask_volumes = np.pad(
                ask_volumes, (0, self.num_levels - len(ask_volumes)), "constant"
            )
    ...
    def compute_order_imbalance(
        self, bid_volumes: np.ndarray, ask_volumes: np.ndarray
    ) -> float:
        """
    ...
    def compute_weighted_mid_price(
        self, best_bid: float, best_ask: float, bid_volume: float, ask_volume: float
    ) -> float:
        """
        ...
        weighted_price = (best_bid * ask_volume + best_ask * bid_volume) / (
    ...
    def compute_second_order_features(
        ...
        # Rates of change
        imbalance_change = (
            current_features["order_imbalance"] - previous_features["order_imbalance"]
        )
        spread_change = current_features["spread"] - previous_features["spread"]
        mid_price_change = (
            current_features["mid_price"] - previous_features["mid_price"]
        )
        ...
        second_order = {
            "imbalance_rate": (
                imbalance_change / time_delta_sec if time_delta_sec > 0 else 0.0
            ),
            "spread_rate": (
                spread_change / time_delta_sec if time_delta_sec > 0 else 0.0
            ),
...
class SpoofingLabelGenerator:
    ...
    def compute_cancellation_ratio(
        self, placed_volume: float, cancelled_volume: float
    ) -> float:
...
if __name__ == "__main__":
    ...
    # Sample order
    order = {"price": 100.5, "volume": 1000, "side": 1, "order_type": "limit"}

    # Extract features
    features = extractor.extract_complete_features(
        lob_snapshot, order, time_since_last_event=5.0
    )
