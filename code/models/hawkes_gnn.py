...
class HawkesProcessEstimator:
        ...
        def negative_log_likelihood(params):
            ...
            for i in range(n):
                ...
                for t in times_i:
                    ...
                    for j in range(n):
                        ...
                        if len(past_events) > 0:
                            kernel_sum = np.sum(
                                self.exponential_kernel(t - past_events, self.beta)
                            )
                ...
                for j in range(n):
                    times_j = event_times.get(j, np.array([]))
                    for t_j in times_j:
                        compensator += alpha[i, j] * (
                            1 - np.exp(-self.beta * (T - t_j))
                        )
    ...
    def compute_adjacency_matrix(self) -> np.ndarray:
        ...
        if self.alpha is None:
            raise ValueError(
                "Parameters not estimated. Call estimate_parameters first."
            )
...
class GraphAttentionLayer(nn.Module):
    ...
    def _prepare_attentional_mechanism_input(self, Wh):
        ...
        return all_combinations_matrix.view(
            batch_size, num_nodes, num_nodes, 2 * out_features
        )
...
class HawkesGNN(nn.Module):
    ...
    def __init__(
        self, input_dim, hidden_dim, output_dim, num_gnn_layers=2, dropout=0.1
    ):
...
class TEN_GNN_Hybrid(nn.Module):
    ...
    def __init__(
        self, ten_model, num_assets, gnn_hidden_dim=128, num_gnn_layers=2, dropout=0.1
    ):
    ...
    def forward(self, multi_asset_data, adjacency_matrix, time_deltas=None):
        ...
        node_features = torch.stack(
            asset_embeddings, dim=1
        )  # (batch_size, num_assets, d_model)

        # Apply GNN with Hawkes adjacency
        graph_embeddings = self.gnn(
            node_features, adjacency_matrix
        )  # (batch_size, num_assets, gnn_hidden_dim)

        # Global pooling across assets
        graph_embeddings = graph_embeddings.transpose(
            1, 2
        )  # (batch_size, gnn_hidden_dim, num_assets)
        pooled = self.global_pool(graph_embeddings).squeeze(
            -1
...
if __name__ == "__main__":
    ...
    gnn = HawkesGNN(
        input_dim=input_dim, hidden_dim=128, output_dim=64, num_gnn_layers=2
    )
